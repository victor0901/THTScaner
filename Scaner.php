        ...
        elseif ($scan == "6")
          {
            ...
            echo $bold . $fgreen" $subdomain)); $resultnmap;
            echo "\n\n";
            echo $bold . $yellow . "[*] Scanning Complete. Press Enter To Continue OR CTRL + C To Stop\n\n";
            trim(fgets(STDIN, 1024));
            goto scanlist;
          ...
          elseif ($scan == "8")
          {
            ...
            $resp  = curl_exec($ch);
            $resp  = str_replace("[", "", str_replace("]", "", str_replace("\"\"", "", str_replace(", ,", "$
            $array = explode(",,", $resp);
            unset($array[0]);
                ...
                if ($wp_version != "")
                  {
                    ...
                    if (strpos($vuln_json, "The page you were looking for doesn't exist (404)") !== false)
                      {
                        echo $red . "[!] Seems like the version detection messed up preety bad! Please report here: https://github.com/Tuhinshubhra/RED_H$
                      }
                    else
                      {
        ...
        elseif ($scan == "A" || $scan == "a")
          {
            ...
            curl_setopt($ch, CURLOPT_POST, 1);
            $resp  = curl_exec($ch);
            $resp  = str_replace("[", "", str_replace("]", "", str_replace("\"\"", "", str_replace(", ,", ",", str_replace("{", "", str_replace("{", "", $
            $array = explode(",,", $resp);
            unset($array[0]);
            ...
            echo "\n\n$bold" . "69:$cln This is the lite version of tge crawler, This will show you the files which returns the http c$
            echo "\n$bold" . "420:$cln This is a little advance one it will show you all the list of files with their http code other $
csel:
