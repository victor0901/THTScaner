...
function getTitle($url) {
  ...
  $title = preg_match('/<title[^>]*>(.*?)<\/title>/ims', $data, $matches) ? $match[1] : 
   return $title;
  }
...
function extract_social_links($sourcecode){
  ...
  */
  ...
  $insta_link_count = 0;                                                                                                                 $yt_link_count = 0;
  $gp_link_count = 0;
  $pint_link_count = 0;
  $github_link_count = 0;
...
function bv_moz_info($url){
  ...
  if (strpos($accessID, " ") !== false OR strpos($secretKey, " ") !== false){
    echo $bold . $red . "\n[!] Some Results Will Be Omited (Please Put Valid MOZ API Keys in config.php file)\n\n";
        ...
        $objURL = $url;
    $flags = "103079231492";
        $reqUrl = "http://lsapi.seomoz.com/linkscape/url-metrics/".urlencode($objURL)."?Cols=".$flags."&AccessID=".$accessID."&Expires$
    $opts = array(
